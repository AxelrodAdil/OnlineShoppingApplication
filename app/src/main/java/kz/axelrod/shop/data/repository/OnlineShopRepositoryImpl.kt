...
import kz.axelrod.shop.data.mapper.CategoryItemsMapper
...
class OnlineShopRepositoryImpl(
    ...
    private val categoryMapper: CategoryItemsMapper
    ...
    override suspend fun getCategoryList(): Response<List<CategoryItem>> = categoryMapper.map(
        try {
            withContext(Dispatchers.IO) {
                val data = service.getCategoryList()
                Response.Success(data)
            }
        } catch (e: Exception) {
            e.catchError()
